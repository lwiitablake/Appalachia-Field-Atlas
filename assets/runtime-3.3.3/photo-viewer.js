import {imageGestures} from './photo-gestures.js';
...
export function zoomButton(root,source,alt){
 const media=root.querySelector('img,canvas'),viewport=document.createElement('div');viewport.className='inline-image-viewport';viewport.tabIndex=0;viewport.setAttribute('aria-label','Image: drag to pan, wheel or pinch to zoom; keyboard arrows pan, plus and minus zoom, zero resets');
 ...
 const gestures=imageGestures(viewport,media,value=>{scale=value;status.textContent=Math.round(scale*100)+'%';minus.disabled=scale<=1;plus.disabled=scale>=4;});
 plus.onclick=()=>gestures.zoom(.5);minus.onclick=()=>gestures.zoom(-.5);reset.onclick=()=>gestures.reset();expand.onclick=()=>openPhoto(typeof source==='function'?source():source,alt);
 controls.append(minus,status,plus,reset,expand);viewport.after(controls);
}
