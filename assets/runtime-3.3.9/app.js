...
import {createPhotoOrganizer} from './photo-organizer.js';
import {submissionEditor} from './photo-submissions.js';
...
import {evidenceFor,coordinates,matchesEvidence} from './editorial.js';
...
const filters={overworld:true,interiors:true,query:'',space:'2480661',categories:['bobblehead','magazine'],view:'all',tag:'',guided:false,photos:false};
let lockAfterMinutes=15;
...
$('.skip-link').onclick=e=>{e.preventDefault();document.body.classList.add('explorer-open');$('#mobile-explorer').setAttribute('aria-expanded','true');$('#search').focus();};
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$('#modal').open&&!document.querySelector('.photo-viewer[open]')){map?.closeTooltip();if(!$('#detail').hidden)minimizeDetail();else if(document.body.classList.contains('explorer-open')){document.body.classList.remove('explorer-open');$('#mobile-explorer').setAttribute('aria-expanded','false');$('#mobile-explorer').focus();}}});
...
function updateSession(){
  ...
  if(name){lastActivity=Date.now();lockTimer=setInterval(()=>{if(Date.now()-lastActivity>lockAfterMinutes*60*1000&&!saveBusy)lock(`Profile locked after ${lockAfterMinutes} minutes of inactivity.`);},15000);}
  render();
}
...
function authDialog(mode='login'){
  ...
  $('#auth-form').onsubmit=async e=>{
    ...
    const pass=String(fd.get('passphrase'));lockAfterMinutes=[15,60,300].includes(Number(fd.get('lockMinutes')))?Number(fd.get('lockMinutes')):15;
...
function categoryUI(){
  $('#categories').innerHTML=Object.entries(CATEGORIES).map(([id,c])=>{
    const count=(id==='custom'?progress.custom:visibleRecords()).filter(r=>r.category===id&&(filters.space==='all'||r.space===filters.space)).length;
  ...
  $('#categories').querySelectorAll('[data-category]').forEach(b=>b.onclick=()=>{const c=b.dataset.category;filters.categories=filters.categories.includes(c)?filters.categories.filter(x=>x!==c):[...filters.categories,c];limit=70;render();$(`#categories [data-category="${c}"]`)?.focus({preventScroll:true});});
...
function applyRegion(){
 if(!map)return;if(regionMask)regionMask.remove();regionMask=null;
 const r=currentRegion(),b=map.spaceId==='2480661'?r?.bounds:null;
 const bounds=b?[mapPoint({x:b[0],y:b[1]},activeSpace()),mapPoint({x:b[2],y:b[3]},activeSpace())]:[[0,0],[4096,4096]];
 map.setMaxBounds(bounds);map.fitBounds(bounds,{animate:false,padding:[12,12]});
 if(b){const [[y1,x1],[y2,x2]]=bounds;regionMask=L.polygon([[[-10000,-10000],[-10000,10000],[10000,10000],[10000,-10000]],[[y1,x1],[y2,x1],[y2,x2],[y1,x2]]],{stroke:false,fillColor:'#252c25',fillOpacity:1,interactive:false}).addTo(map);}
 $('#map-name').textContent=b?r.name:activeSpace().name;
}
function renderNearby(origin){
 const category=$('#nearby-category')?.value||'all';
 const items=nearbyRecords(allRecords(),origin,filters,progress,guides,category);
 $('#nearby-results').innerHTML=items.length?items.map(({record:r,distance,match})=>`<button class="nearby-result" data-nearby="${r.id}"><strong>${esc(r.area)}</strong><span>${esc(CATEGORIES[r.category].singular)} · ${Math.round(distance).toLocaleString()} units${match?' · Search match':''}</span></button>`).join(''):'<p>No matching locations within 20,000 game units on this map.</p>';
 $('#nearby-results').querySelectorAll('[data-nearby]').forEach(b=>b.onclick=()=>selectRecord(allRecords().find(r=>r.id===b.dataset.nearby),true));
}
function visibleRecords(){return displayedRecords(allRecords(),atlas.spaces,entrances,filters);}
...
function render(){
  ...
  results=visibleRecords().filter(r=>filters.categories.includes(r.category)&&matches(r,{...filters,guided:false},progress,guides)&&matchesEvidence(r,guides,details,filters.photos,filters.guided)&&within(r,filters.space==='2480661'?currentRegion()?.bounds:null)).sort((a,b)=>a.area.localeCompare(b.area)||a.category.localeCompare(b.category)||a.id.localeCompare(b.id));
  $('#result-count').textContent=`${results.length.toLocaleString()} discoveries`;
  const shown=results.slice(0,limit);
  $('#results').innerHTML=shown.length?shown.map(r=>{const c=CATEGORIES[r.category],p=progress.items[r.id]||{};return `<button class="result ${selected?.id===r.id?'selected':''}" data-id="${r.id}" aria-label="${esc(r.area)} · ${esc(r.name)}${r.formId?` · ${r.formId.slice(-4)}`:''} · ${esc(evidenceFor(r,guides,details).label)}" style="--cat:${r.interiorProxy?'#555b62':c.color}"><span class="category-symbol" aria-hidden="true">${categoryIcon(r.category)}</span><span><strong>${esc(r.area==='Personal marker'?r.name:r.area)}</strong><small>${r.interiorProxy?'↳ Inside / submap · ':''}${esc(LOCATION_CATEGORIES.includes(r.category)?CATEGORIES[r.category].singular:r.name)}${r.formId?` · ${r.formId.slice(-4)}`:''} ${filters.space==='all'?`· ${esc(atlas.spaces[r.space]?.name||'Unknown map')}`:''}</small></span><span class="status" aria-label="${p.star?'Starred ':''}${p.done?'Checked ':''}${p.tags?.length?'Tagged':''}">${p.star?'★ ':''}${p.done?'✓ ':''}${p.tags?.length?'●':''}</span></button>`;}).join(''):'<p class="empty">No discoveries match these filters. Try another map, a broader search, or All layers.</p>';
...
function tooltip(r){
  ...
  p.textContent=r.interiorProxy?`Inside ${atlas.spaces[r.interiorSpace].name}. Gray marker shows an entrance, not the item's outdoor position.`:coordinates(r);hint.textContent=evidenceFor(r,guides,details).label+' · Click for details';d.append(strong,p,hint);return d;
}
function pinPoint(r){
 const pos=mapPoint(r,activeSpace());if(!r.interiorProxy)return pos;
 const types=['bobblehead','magazine','caps','armor','fusion','lunchbox','cooler'];
 const angle=types.indexOf(r.category)*Math.PI*2/types.length,px=map.project(pos,map.getZoom());
 const ll=map.unproject([px.x+Math.cos(angle)*30,px.y+Math.sin(angle)*30],map.getZoom());return [ll.lat,ll.lng];
}
function renderPins(){
  ...
  for(const r of onMap){const pos=pinPoint(r),px=map.project(pos,map.getZoom());const key=r.interiorProxy?`inside:${r.interiorSpace}:${r.category}`:map.getZoom()<2?`${Math.floor(px.x/38)}:${Math.floor(px.y/38)}`:r.id;const group=groups.get(key)||[];group.push(r);groups.set(key,group);}
  for(const group of groups.values()){
    if(group.length>1){
      const points=group.map(r=>pinPoint(r));const center=[points.reduce((s,p)=>s+p[0],0)/points.length,points.reduce((s,p)=>s+p[1],0)/points.length];
      const cluster=L.marker(center,{keyboard:false,icon:L.divIcon({className:`pin cluster ${group.every(r=>r.interiorProxy)?'interior-resource':''} ${group.some(r=>evidenceFor(r,guides,details).photos.length)?'has-photo':''} ${group.some(r=>{const e=evidenceFor(r,guides,details);return e.text||e.area;})?'has-text':''}`,html:`<span style="--cat:${group.every(r=>r.interiorProxy)?'#555b62':'#30483e'}">${group.every(r=>r.interiorProxy)?categoryIcon(group[0].category):group.length}</span>${group.every(r=>r.interiorProxy)?`<b class="interior-count">${group.length}</b>`:''}`,iconSize:[31,31],iconAnchor:[15.5,15.5]})}).addTo(markers);
      ...
      if(group.every(r=>r.interiorProxy)){cluster.unbindTooltip();const hint=document.createElement('span');hint.textContent=`${group.length} ${CATEGORIES[group[0].category].name} inside ${atlas.spaces[group[0].interiorSpace].name}. Open interior list.`;cluster.bindTooltip(hint);cluster.on('click',()=>{modal(`<h2 id="modal-title">Inside ${esc(atlas.spaces[group[0].interiorSpace].name)}</h2><p>These resources are in a separate interior. Gray category icons are spread slightly around the entrance so they do not cover one another. The gray marker shows a sourced entrance; access may depend on quests or events.</p><div class="interior-list">${group.map(r=>`<button data-interior-record="${r.id}">${esc(r.name)} · ${esc(r.formId)}</button>`).join('')}</div>`);$('#modal-content').querySelectorAll('[data-interior-record]').forEach(b=>b.onclick=()=>{closeModal();selectRecord(allRecords().find(r=>r.id===b.dataset.interiorRecord),true);});});continue;}
      cluster.on('click',()=>map.fitBounds(L.latLngBounds(points).pad(.3),{maxZoom:Math.max(2,map.getZoom()+1.5),padding:[50,90]}));continue;
    }
    const r=group[0];
    const c=CATEGORIES[r.category],p=progress.items[r.id]||{},pos=pinPoint(r);
    const icon=L.divIcon({className:`pin ${r.interiorProxy?'interior-resource':''} ${evidenceFor(r,guides,details).classes} ${p.done?'checked':''} ${p.star?'starred':''} ${p.tags?.length?'tagged':''} ${selected?.id===r.id?'selected':''}`,html:`<span style="--cat:${r.interiorProxy?'#555b62':c.color}">${categoryIcon(r.category)}</span>`,iconSize:[25,25],iconAnchor:[12.5,12.5]});
    const marker=L.marker(pos,{icon,keyboard:false,title:`${r.name} · ${r.area}`}).addTo(markers).on('click',()=>selectRecord(allRecords().find(item=>item.id===r.id),true));
...
function selectRecord(r,focus){
  ...
  if(organizerPick){const done=organizerPick;organizerPick=null;done();}
}
...
let imageGeneration=0,detailMinimized=false,sheetSwipeController;
function minimizeDetail(){if(!selected)return;const restoreFocus=$('#detail').contains(document.activeElement);detailMinimized=true;$('#detail').hidden=true;$('#restore-detail').hidden=false;$('#restore-detail').textContent='Open notes · '+selected.area;if(restoreFocus)$('#restore-detail').focus({preventScroll:true});map?.invalidateSize({animate:false});}
...
function renderDetail(){
  ...
  header.onpointerup=e=>{if(headerStart&&shouldMinimize(e.clientX-headerStart.x,e.clientY-headerStart.y))closeDetail();headerStart=null;};header.onpointercancel=()=>headerStart=null;
  // iOS can cancel Pointer Events when it starts native scrolling. Reserve the header for dismissal.
  let touchStart=null;
  const sheet=$('#detail');
  sheetSwipeController?.abort();sheetSwipeController=new AbortController();const signal=sheetSwipeController.signal;
  sheet.addEventListener('touchstart',e=>{if(e.target.closest('button,a,input,select,textarea,.image-gallery')||e.touches.length!==1)return;if(header.contains(e.target)||sheet.scrollTop<=0)touchStart={x:e.touches[0].clientX,y:e.touches[0].clientY};},{passive:true,signal});
  sheet.addEventListener('touchmove',e=>{if(touchStart&&e.touches.length===1&&e.touches[0].clientY>touchStart.y)e.preventDefault();},{passive:false,signal});
  sheet.addEventListener('touchend',e=>{if(touchStart&&e.changedTouches.length&&shouldMinimize(e.changedTouches[0].clientX-touchStart.x,e.changedTouches[0].clientY-touchStart.y))closeDetail();touchStart=null;},{signal});
  sheet.addEventListener('touchcancel',()=>touchStart=null,{signal});

  $('#submit-photos').onclick=()=>submissionEditor(r,repositoryFor(location,siteConfig.repository),modal);
  ...
  mountGallery($('#image-gallery'),evidenceFor(r,guides,details).photos,root=>{
    ...
    drawCrop(r,space);zoomButton(root,()=>$('#location-image').toDataURL(),'Overhead map placeholder; crosshair marks the selected point.');
  });
}
...
function addMarkerDialog(latlng){
  ...
  $('#marker-form').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),name=String(fd.get('name')).trim();if(!name){failForm(Error('Enter a name.'));return;}
    ...
    if(await mutate(n=>{if(n.custom.length>=2000)throw Error('The limit is 2,000 personal markers.');n.custom.push(r);const st=itemState(n,r.id);st.tags=[String(fd.get('tag'))];})){
      closeModal();if(!filters.categories.includes('custom'))filters.categories.push('custom');filters.view='all';$('#view').value='all';selectRecord(allRecords().find(item=>item.id===r.id),true);toast('Personal marker saved.');
    }
  };
}
...
document.addEventListener('click',e=>{
 ...
 if(document.body.classList.contains('explorer-open')&&!e.target.closest('#explorer-panel,#mobile-explorer,.skip-link')){document.body.classList.remove('explorer-open');$('#mobile-explorer').setAttribute('aria-expanded','false');}
...
async function start(){
  try{
    ...
    for(const key of ['overworld','interiors'])$('#show-'+key).onchange=e=>{filters[key]=e.target.checked;limit=70;render();};
    $('#photos-only').onchange=e=>{filters.photos=e.target.checked;limit=70;render();};
    ...
    $('#map-key').onclick=mapKey;$('#badge-preview').onclick=badgePreview;$('#help-button').onclick=helpDialog;$('#sources-button').onclick=sourcesDialog;$('#photo-organizer').onclick=()=>{organizer??=createPhotoOrganizer({modal,closeModal,getSelected:()=>atlas.records.find(r=>r.id===selected?.id),records:()=>atlas.records,pickLocation:done=>{organizerPick=done;minimizeDetail();toast('Select a map pin or search result, then confirm the photo assignment.');},submit:(r,files)=>submissionEditor(r,repositoryFor(location,siteConfig.repository),modal,files)});organizer.open();};$('#moderator-tools').onclick=()=>{const repo=repositoryFor(location,siteConfig.repository);modal(`<h2 id="modal-title">Moderator review</h2><p>GitHub verifies your account and repository permissions. Only configured moderators with repository write access can publish. Local profiles do not grant moderation rights.</p><p>${external('https://github.com/'+repo+'/issues?q=is%3Aissue+is%3Aopen+%22%5BAtlas+photos%5D%22','Open photo submission queue')}</p><p>${external('https://github.com/'+repo+'/actions/workflows/review-photos.yml','Open protected review & publication workflow')}</p><p>First run with Publish unchecked. Inspect the issue and copy the review hash from the workflow summary. Run again with Publish checked and that hash. No approval occurs just by opening these links.</p><p>${external('PHOTO-WORKFLOW.md','Moderator and screenshot guide')}</p>`);};
