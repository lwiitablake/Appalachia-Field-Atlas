...
export const fileKey=f=>JSON.stringify([f.webkitRelativePath||f.name,f.size,f.lastModified]);
...
export function createPhotoOrganizer({modal,closeModal,getSelected,pickLocation,records,submit}){
 ...
 function draw(){
  ...
  const shown=files.filter(f=>{const id=assignments[fileKey(f)];return (filter==='all'||(filter==='unassigned'?!id:filter==='assigned'?!!id:id===r?.id))&&(f.webkitRelativePath||f.name).toLowerCase().includes(query.toLowerCase());});
