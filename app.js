...
let backgroundPreference='satellite',backgroundOpacity=1,showLocationLabels=false,areaOutline;
...
function updateSession(){
  ...
  $('#local-status').textContent=name?'Encrypted on this browser · back up regularly.':'Browse freely. Log in locally to save.';
...
function renderBackground(){
  ...
  if(b.image){overlay=L.imageOverlay(b.image,[[0,0],[4096,4096]],{opacity:backgroundOpacity,alt:`${b.name}: ${s.name}. Mappalachia; © Bethesda / ZeniMax.`}).addTo(map);overlay.on('error',()=>toast('Background failed to load. Try another background; points remain searchable.'));}
...
function tooltip(r){
  const d=document.createElement('div'),strong=document.createElement('strong'),p=document.createElement('div');strong.textContent=`${r.name} · ${r.area}`;p.textContent=r.directions;d.append(strong,p);return d;
}
function renderPins(){
  ...
  for(const group of groups.values()){
    ...
    const icon=L.divIcon({className:`pin ${p.done?'checked':''} ${p.star?'starred':''} ${p.tags?.length?'tagged':''} ${selected?.id===r.id?'selected':''}`,html:`<span style="--cat:${c.color}">${c.symbol}</span>`,iconSize:[25,25],iconAnchor:[12.5,12.5]});
    const marker=L.marker(pos,{icon,keyboard:false,title:`${r.name} · ${r.area}`}).addTo(markers).on('click',()=>selectRecord(r,false));
    marker.bindTooltip(tooltip(r),{direction:'top',offset:[0,-15]});
...
function selectRecord(r,focus){
  if(!r)return;detailReturn=document.activeElement;selected=r;setMap(r.space);render();
...
function closeDetail(){selected=null;$('#detail').hidden=true;map?.invalidateSize({animate:false});renderPins();detailReturn?.focus?.({preventScroll:true});}
...
async function start(){
  try{
    const responses=await Promise.all([fetch('data/atlas.json'),fetch('data/guides.json')]);
    if(responses.some(r=>!r.ok))throw Error('The atlas files could not be loaded.');
    [atlas,guides]=await Promise.all(responses.map(r=>r.json()));
    ...
    $('#edition').textContent=`DATA EDITION · ${atlas.meta.released} / ${atlas.meta.gameVersion}`;
    ...
    $('#help-button').onclick=helpDialog;$('#sources-button').onclick=sourcesDialog;
  ...
  }catch(e){$('#results').innerHTML=`<p class="empty">${esc(e.message)} Open this app through GitHub Pages or the included local server, and reload.</p>`;$('#result-count').textContent='Unable to load atlas';toast('Could not load the atlas. Check the included setup instructions.');console.error(e);}
}
start();
