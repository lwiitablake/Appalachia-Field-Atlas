...
function badgePreview(){
 modal(`<h2 id="modal-title">Photo & guide badge preview</h2><p>Style examples only. These samples are not real locations and do not claim photographs exist for any pin.</p><div class="badge-samples">${[['','Map only'],['has-photo','Spot photo'],['has-text','Written guide'],['has-photo has-text','Both: double outline']].map(([cl,label])=>`<div><span class="pin ${cl}" style="--cat:#ae542a"><span>${categoryIcon('bobblehead')}</span></span><strong>${label}</strong></div>`).join('')}</div><h3>Where are the screenshots?</h3><p>The app currently has overhead map crops, not licensed close-up spot photos. Blue outlines are reserved for photos added with verified reuse terms and pin correspondence.</p><p>For a real external example, the Landview Lighthouse article’s gallery shows potential bobblehead spots in the lighthouse window and upstairs in the house, plus magazine spots.</p><p>${external('https://fallout.fandom.com/wiki/Landview_Lighthouse#Gallery','View Landview Lighthouse screenshot gallery')}</p><p class="form-help">External reference only. Those photographs have not been copied into this app or assumed to share the wiki text license.</p>`);
}
function mapKey(){
...
function editorialHTML(r){
 ...
 return `<p class="evidence-label">${esc(e.label)}</p>${gallery}${e.photos.map(photo=>`<figure class="spot-photo"><img src="${esc(photo.path)}" alt="${esc(photo.alt)}" loading="lazy"><figcaption>${external(photo.source,photo.credit)} · ${esc(photo.license)}</figcaption></figure>`).join('')}${e.text?`<section class="directions-card"><p class="eyebrow">WHERE TO LOOK</p><h3>Detailed spot description</h3><p>${esc(e.entry.description)}</p><p class="form-help">${external(e.entry.source,e.entry.credit)} · ${esc(e.entry.license)}</p></section>`:''}`;
}
...
async function start(){
  try{
    ...
    $('#edition').textContent=`V3.1 · ${atlas.meta.released} / ${atlas.meta.gameVersion}`;
  ...
  }catch(e){$('#edition').textContent='V3.1 · Setup incomplete';$('#results').innerHTML=`<p class="empty">${esc(e.message)} Check that data/, vendor/ and assets/ were uploaded as folders beside index.html. V3 also requires data/details.json, data/community.json and site-config.json.</p>`;$('#result-count').textContent='Unable to load atlas';toast('Could not load the atlas. Check the included setup instructions.');console.error(e);}
}
start();
