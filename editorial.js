export function evidenceFor(record,guides,details={}){
 ...
 return {photos,text,area,entry:exact,classes:[photos.length?'has-photo':'',(text||area)?'has-text':''].filter(Boolean).join(' '),label:[photos.length?'Sourced spot photo':'',text?'Detailed pin description':area?'Written area guide (not pin-verified)':''].filter(Boolean).join(' · ')||'Coordinates and overhead map only'};
}
